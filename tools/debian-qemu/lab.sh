...
test_install() {
    ...
    ssh_guest "test ! -e /etc/systemd/system/nucore.service && test ! -e /etc/nucore-portable/session.conf && test ! -e /var/lib/nucore-portable/install-mode && test \"\$(getent passwd cabinet | cut -d: -f7)\" = /bin/bash"
    stop_vm
